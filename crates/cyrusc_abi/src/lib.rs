...
use cyrusc_sema::monomorph::MonomorphID;

/// Makes an ABI-safe function name based on visibility and module context.
pub fn make_func_abi_name(module_name: &str, func_name: &str, vis: AccessSpecifier) -> String {
    match vis {
        AccessSpecifier::Extern | AccessSpecifier::PublicExtern | _ if func_name == "main" => func_name.to_string(),
        _ => format!("{module_name}.{func_name}"),
    }
}

/// Makes an ABI-safe global variable name, respecting external linkage rules.
pub fn make_global_var_abi_name(module_name: &str, var_name: &str, vis: &AccessSpecifier) -> String {
    match vis {
        AccessSpecifier::Extern | AccessSpecifier::PublicExtern => var_name.to_string(),
        _ => format!("{module_name}.{var_name}"),
    }
}
...
fn make_monomorphic_type_abi_name(module_name: &str, type_name: &str, monomorph_id: MonomorphID) -> String {
...
pub fn make_monomorphic_struct_abi_name(module_name: &str, struct_name: &str, monomorph_id: MonomorphID) -> String {
    make_monomorphic_type_abi_name(module_name, struct_name, monomorph_id)
}

/// Makes an ABI-safe name for a monomorphized union.
pub fn make_monomorphic_union_abi_name(module_name: &str, union_name: &str, monomorph_id: MonomorphID) -> String {
    make_monomorphic_type_abi_name(module_name, union_name, monomorph_id)
}

/// Makes an ABI-safe name for a monomorphized enum.
pub fn make_monomorphic_enum_abi_name(module_name: &str, enum_name: &str, monomorph_id: MonomorphID) -> String {
