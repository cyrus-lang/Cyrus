    ...
    pub(crate) fn check_type_mismatch(
        ...
        match (value_type.const_inner().clone(), target_type.const_inner().clone()) {
            ...
            (
                ...
                let symbol_entry = self.query.get_symbol_entry(struct_id).unwrap();
            ...
            (
                ...
                let symbol_entry = self.query.get_symbol_entry(union_id).unwrap();
            ...
            (SemanticType::DynamicType(dynamic_type), SemanticType::Interface(interface_type)) => {
                dynamic_type.interface_id == interface_type.symbol_id
            }
    ...
    pub(crate) fn check_explicit_typecast(&mut self, value_type: SemanticType, target_type: SemanticType) -> bool {
        match (value_type, target_type) {
            ...
            (SemanticType::ResolvedSymbol(ResolvedSymbol::Enum(enum_id)), SemanticType::PlainType(plain_type)) => {
                let symbol_entry = self.query.get_symbol_entry(enum_id).unwrap();
