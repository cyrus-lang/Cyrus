...
impl<'a> AnalysisContext<'a> {
    ...
    fn get_typedef(
        ...
        let local_or_global = self.get_symbol_entry(local_scope_opt, symbol_id, loc.clone())?;
        let typedef_opt = match &local_or_global {
            LocalOrGlobalSymbol::LocalSymbol(local_symbol) => local_symbol.as_typedef(),
            LocalOrGlobalSymbol::GlobalSymbol(symbol_entry) => symbol_entry.as_typedef(),
        };

        match typedef_opt {
            Some(resolved_typedef) => Some(resolved_typedef.clone()),
            None => {
                self.report_non_type_symbol(symbol_id, loc.clone());
                None
            }
