...
impl<'ll> IRBuilderCtx<'ll> {
    ...
    pub fn emit_program_tree(&self, cir_program_tree: &CIRProgramTree) {
        for cir_stmt in &cir_program_tree.body {
            self.emit_stmt(cir_stmt);
        }
    }

    pub(crate) fn emit_stmt(&self, cir_stmt: &CIRStmt) {
        match cir_stmt {
    ...
    pub(crate) fn emit_block(&self, cir_block: &CIRBlockStmt) {
    ...
    fn emit_global_var(&self, cir_global_var: &CIRGlobalVarStmt) -> Option<GlobalValue<'ll>> {
        let irreg = self.irreg.borrow();

        if let Some(local_ir_value) = irreg.get(cir_global_var.irv_id) {
            return local_ir_value.as_global().cloned();
        }
        ...
        // let mut irreg = self.irreg.borrow_mut();
        // irreg.insert(cir_global_var.irv_id, LocalIRValue::Global(ptr));
        todo!();
    }
}
