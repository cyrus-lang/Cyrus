...
%{
    ...
    extern std::shared_ptr<ASTProgram> astProgram = nullptr;
%}
...
%code requires {
    ...
    using NodeListPtr = std::shared_ptr<std::vector<ASTNodePtr>>;
    using ASTTypeSpecifierPtr = std::shared_ptr<ASTTypeSpecifier>;

    using ASTFunctionParametersPtr = std::shared_ptr<ASTFunctionParameters>;
    using ASTFunctionDefinitionPtr = std::shared_ptr<ASTFunctionDefinition>;

    using ASTStructFieldPtr = std::shared_ptr<ASTStructField>;
    using StructMembersAndMethodsPtr = std::shared_ptr<std::pair<std::vector<ASTStructField>, std::vector<ASTFunctionDefinition>>>;
    using FieldInitializerListPtr = std::shared_ptr<std::vector<std::pair<std::string, ASTNodePtr>>>;

    using EnumData = std::variant<ASTEnumVariant, std::pair<std::string, std::optional<ASTNodePtr>>, ASTFunctionDefinition>;
    using EnumDataPtr = std::shared_ptr<EnumData>; 
    using EnumDataListPtr = std::shared_ptr<std::vector<EnumData>>; 
    using ASTEnumVariantItemPtr = std::shared_ptr<ASTEnumVariantItem>;
    using EnumVariantItemsListPtr = std::shared_ptr<std::vector<ASTEnumVariantItemPtr>>; 
}

%type <int> TOK_INTEGER_CONSTANT
%type <float> TOK_FLOAT_CONSTANT
%type <double> TOK_DOUBLE_CONSTANT
%type <std::string> TOK_STRING_CONSTANT 
%type <std::string> TOK_IDENTIFIER

%nterm <ASTAssignment::Operator> assignmentOperator
%nterm <ASTStorageClassSpecifier> storageClassSpecifier
%nterm <ASTUnaryExpression::Operator> unaryOperator
%nterm <ASTAccessSpecifier> accessSpecifier
%nterm <ASTNodePtr> node
%nterm <ASTTypeSpecifierPtr> typeSpecifier
%nterm <ASTFunctionParametersPtr> paramsListPtr
%nterm <ASTFunctionDefinitionPtr> funcDef
%nterm <ASTStructFieldPtr> structField
%nterm <ASTEnumVariantItemPtr> enumVariantItem
%nterm <EnumDataPtr> enumData
%nterm <StructMembersAndMethodsPtr> structMembersAndMethods
%nterm <FieldInitializerListPtr> fieldInitializerList
%nterm <std::shared_ptr<std::pair<std::string, ASTNodePtr>>> structFieldInitPair
%nterm <EnumVariantItemsListPtr> enumVariantItemsList
%nterm <NodeListPtr> nodeListPtr
%nterm <EnumDataListPtr> enumDataList 
%nterm <EnumData> enumerator 

%token TOK_IMPORT TOK_TYPEDEF TOK_FUNCTION TOK_EXTERN TOK_INLINE TOK_HASH
%token TOK_CLASS TOK_PUBLIC TOK_PRIVATE TOK_INTERFACE TOK_ABSTRACT TOK_VIRTUAL TOK_OVERRIDE TOK_PROTECTED
%token TOK_UINT128 TOK_VOID TOK_CHAR TOK_BYTE TOK_STRING TOK_FLOAT32 TOK_FLOAT64 TOK_FLOAT128 TOK_BOOL TOK_ERROR
%token TOK_INT TOK_INT8 TOK_INT16 TOK_INT32 TOK_INT64 TOK_INT128 TOK_UINT TOK_UINT8 TOK_UINT16 TOK_UINT32 TOK_UINT64
%token TOK_CASE TOK_DEFAULT TOK_IF TOK_ELSE TOK_SWITCH TOK_WHILE TOK_DO TOK_FOR TOK_CONTINUE TOK_BREAK TOK_RETURN
%token TOK_PTR_OP TOK_INC_OP TOK_DEC_OP TOK_LEFT_OP TOK_RIGHT_OP TOK_LE_OP TOK_GE_OP TOK_EQ_OP TOK_NE_OP
%token TOK_AND_OP TOK_OR_OP TOK_MUL_ASSIGN TOK_DIV_ASSIGN TOK_MOD_ASSIGN TOK_ADD_ASSIGN
%token TOK_XOR_ASSIGN TOK_OR_ASSIGN TOK_STRUCT TOK_ENUM TOK_ELLIPSIS TOK_CONST
%token TOK_SUB_ASSIGN TOK_LEFT_ASSIGN TOK_RIGHT_ASSIGN TOK_AND_ASSIGN
...
%initial-action
{
    astProgram = std::make_shared<ASTProgram>();
}
...
//                                                                     ASTStatementList* statementList = static_cast<ASTStatementList*>($2);
//                                                                     for (auto &&item : static_cast<ASTStatementList*>($3)) {
...
//                                                                 if ($$) 
//                                                                 {
//                                                                     ASTStatementList* statementList = static_cast<ASTStatementList *>($$);
//                                                                     statementList->addStatement($2);
//                                                                 } 
//                                                                 else 
//                                                                 {
